class SegmentTreeBeats:
    '''
    Constraints : N <= 2*10^5, |a[i]| <= 10^12
    range_chmin: For each [l, r), a[i] = min(a[i], v)
    range_chmax: For each [l, r), a[i] = max(a[i], v)
    range_add:   For each [l, r), a[i] = a[i] + v
    get_sum:     Returns sum [l, r)
    '''
    pINF = 1<<60
    nINF = -1<<60
