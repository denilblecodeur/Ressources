...
int main(){
	...
	for(int i=0; i<m; i++){
		...
		cin >> a >> b >> d >> c;
		a--; b--;
		dinic.add_arc(a, b, d, c);
		ideg[b]++;
		odeg[a]++;
	}
	for(int v=0; v<n; v++){
		if(ideg[v] == 0) dinic.add_arc(s, v, 0, INT_MAX);
		if(odeg[v] == 0) dinic.add_arc(v, t, 0, INT_MAX);
	}
	ll flow_value = dinic.compute(s, t);
	cout << flow_value << endl;
}
